	...
	<div class="container-fluid border rounded p-4 m-2 col-lg-4 mx-auto">
		...
		<?php if (!empty($row)) : ?>
			...
			<form method="post" enctype="multipart/form-data">
				...
				<h5 style="font-size: 30px;" class="text-dark"><i class="fa fa-hamburger"></i> Edit Product</h5>
				...
				<div class="mb-3">
					<label for="categoryControlInput1" class="form-label">Category</label>
					<select aria-placeholder="Select Product Category" name="category" aria-label="Default select example" class="form-select  <?= !empty($errors['category']) ? 'border-danger' : '' ?>">
				...
				<div class="mb-3">
					...
					<select aria-placeholder="Select Product Category" name="prod_code" aria-label="Default select example" class="form-select  <?= !empty($errors['prod_code']) ? 'border-danger' : '' ?>">
				...
				<div class="mb-3">
					<label for="discountControlInput1" class="form-label">Discount <small class="text-muted">(optional)</small></label>
					<input value="<?= set_value('discount', $row['discount']) ?>" name="discount" type="text" class="form-control <?= !empty($errors['discount']) ? 'border-danger' : '' ?>" id="discountControlInput1" placeholder="Product discount">
