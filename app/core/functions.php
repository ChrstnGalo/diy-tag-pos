...
function login($input)
{
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (!empty($_POST['rfid']) && !empty($_POST['pin_number'])) {
			// Handle RFID and PIN login
			$rfid = $_POST['rfid'];
			$pin_number = $_POST['pin_number'];

			$row = $user->where(['rfid' => $rfid, 'pin_number' => $pin_number]);

			if (!empty($row) && $rfid == $row[0]['rfid'] && $pin_number == $row[0]['pin_number']) {
				authenticate($row[0]);
				redirect('home');
			} else {
				$errors['rfid'] = "RFID and PIN number do not match";
			}
		} else if (!empty($_POST['unique_num']) && isset($_POST['pin_number'])) {
			...
			if (!empty($row) && $unique_num == $row[0]['unique_num'] && $pin_number == $row[0]['pin_number']) {
				authenticate($row[0]);
				redirect('home');
			} else {
